				...
				for (int i = 0; i < NUM_TESTS; i++) {
					String methodName = ApptRandomTest.RandomSelectMethod(random);
					   if (methodName.equals("Valid")){
   						   int hour=(int) ValuesGenerator.getRandomIntBetween(random, -10, 27);
						   appt.setStartHour(hour);						   
	   					   int minute=(int) ValuesGenerator.getRandomIntBetween(random, -10, 70);
						   appt.setStartMinute(minute);						   
		   				   int day=(int) ValuesGenerator.getRandomIntBetween(random, -10, 40);
						   appt.setStartDay(day);						   
			   			   int month=(int) ValuesGenerator.getRandomIntBetween(random, -10, 20);
